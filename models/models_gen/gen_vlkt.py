...
from datasets.custom_text_label import custom_hoi_text_label, custom_obj_text_label, custom_unseen_index
...
class GEN_VLKT(nn.Module):
    def __init__(self, backbone, transformer, num_queries, aux_loss=False, args=None):
        ...
        if self.args.dataset_file == 'hico':
            # Dùng custom labels thay thế HICO-DET gốc
            hoi_text_label = custom_hoi_text_label
            obj_text_label = custom_obj_text_label
            unseen_index = custom_unseen_index
        ...
        num_obj_classes = args.num_obj_classes  # dùng args thay vì tính từ CLIP labels
        ...
        if args.with_obj_clip_label:
            ...
            # Thêm embedding zeros cho background class để khớp num_obj_classes + 1
            bg_embedding = torch.zeros(1, obj_clip_label.shape[1], device=obj_clip_label.device)
            obj_clip_label_with_bg = torch.cat([obj_clip_label, bg_embedding], dim=0)
            obj_clip_label_with_bg = obj_clip_label_with_bg / (obj_clip_label_with_bg.norm(dim=-1, keepdim=True) + 1e-8)
            self.obj_visual_projection.weight.data = obj_clip_label_with_bg
        else:
